...
def isMonotonic(data):
    data=list(data)
    temp=sorted(data)
    return temp==data or temp==data[::-1]


def CurFit(data):
    from scipy import optimize

    # if np.var(sample_pts[:,0])>np.var(sample_pts[:,1]):
    x = data[0]
    y = data[1]
    iteration = 1000
    perr_min1 = np.inf
...
def RoadGeoConstruction(p_best, x_start, x_end, func_type, if_reverse):
    x_vec = [x_start, p_best[0], x_end]
    sample_n = 40000  # total sample points
    seg_n = 5000  # section count
    ...
    x_pts = [xxx, xxx_line[0], xxx_line[-1]]
    y_pts = [yvals, yyy_line[0], yyy_line[-1]]

    kk = np.gradient(yvals)
    kkk = np.gradient(kk)
    if if_reverse:
        hdg_line =  math.pi / 2 -math.atan(p_best[2]) 
    else:
    ...
    for n in range(0, seg_n):
        ...
        if if_reverse:
            hdg_cur[0, n] = math.atan(2 * p_best[4] * xxx[pts_seg * n] + p_best[3]) + math.pi / 2
            cur_start_list[0, n] = startPts[1]
            cur_start_list[1, n] = startPts[0]
        else:
    ...
    if if_reverse:
        line_start = [yyy_line[0], xxx_line[0]]
    else:
        line_start = [xxx_line[0], yyy_line[0]]
    line_len = np.sqrt((xxx_line[0] - xxx_line[-1]) ** 2 + (yyy_line[0] - yyy_line[-1]) ** 2)

    return cur_list, cur_len_list, cur_start_list, hdg_cur, line_len, line_start, hdg_line, x_pts, y_pts

def CurRoadNode_new(road_id,is_single_road,far_near_end,driving_lane_count,side_walk_count,function_flag,cur_array, cur_len, cur_position, cur_hdg, line_len, line_start, line_hdg):
    #_,seg_list = CalculateCur(road_samples,2,30)
    
    road_length=np.sum(cur_len)+line_len
    road_node=ET.Element('road',name="Road"+" "+str(road_id) ,length=str(road_length), id=str(road_id), junction="-1")
    link=ET.SubElement(road_node,'link')
    if not is_single_road:
        if far_near_end=='far':
            successor=ET.SubElement(link,'successor',elementType="junction" ,elementId="4")
        else:
            predecessor=ET.SubElement(link,'predecessor',elementType="junction" ,elementId="4")
...
def GenerateRoad(sample_pts,filename,roadID,driving_lane_count,sidewalk_lane_count):
    ...
    if isMonotonic(sample_pts[:,0]):
        x=sample_pts[:,0]
        y=sample_pts[:,1]
        if_reverse = False
    elif isMonotonic(sample_pts[:,1]):
        x=sample_pts[:,1]
        y=sample_pts[:,0]
        if_reverse = True
    ppp1,function_flag=CurFit([x,y])
    Cur_array, Cur_len, Cur_position, Cur_hdg, Line_len, Line_start, Line_hdg,x_pts,y_pts= RoadGeoConstruction(ppp1,x[0],
                                                                                                   x[-1],
                                                                                                   function_flag,if_reverse)
    roadnode=CurRoadNode_new(roadID,True,'far',driving_lane_count,sidewalk_lane_count,function_flag,
    ...
    for i in range(4):
        if i==edit_flag:
            ...
            Cur_array, Cur_len, Cur_position, Cur_hdg, Line_len, Line_start, Line_hdg= RoadGeoConstruction(ppp1,sample_pts[0,0],
                                                                                                   sample_pts[-1,0],
                                                                                                   function_flag)
            roadnode=CurRoadNode_new(i,False,SPts_position[i],driving_lane_counts[i,:],[1,1],function_flag,
                             Cur_array,Cur_len, Cur_position, Cur_hdg, Line_len, Line_start, Line_hdg)
