...
class JDETracker(object):
    def __init__(self, reid_ckpt, frame_rate=30):
        #self.model = Darknet(opt.cfg, nID=14455)
        # load_darknet_weights(self.model, opt.weights)
        #self.model.load_state_dict(torch.load(opt.weights, map_location='cpu')['model'], strict=False)
        #self.model.cuda().eval()
        self.feature_extractor = FeatureExtractor(reid_ckpt)
        ...
        self.frame_id = 0
        self.det_thresh = 0.5#opt.conf_thres
        self.buffer_size = int(frame_rate / 30.0 * 30)#opt.track_buffer=30)
    ...
    def update(self, bgr, tlbrs, confidence, labels):#im_blob, img0):
        """
        ...
        if self.image_width<=0 or self.image_height <=0: 
            self.image_width = bgr.shape[1]
            self.image_height = bgr.shape[0]
        ...
        if len(pred) > 0:
            ...
            detections = []
        '''
        embeddings = self.feature_extractor.extract(bgr, tlbrs.copy())
        detections = []
        for i in range(tlbrs.shape[0]): 
            detections.append(STrack(STrack.tlbr_to_tlwh(tlbrs[i,:]), confidence[i],labels[i], embeddings[i], 30))
        ...
        self.output_stracks = [track for track in self.tracked_stracks if track.state==TrackState.Tracked]
    ...
    def get_tracklets(self): 
        results = []        
        for trk in self.output_stracks:#valid_tracks:
            tlwh = trk.tlwh
            left = max(0, tlwh[0])
            top = max(0, tlwh[1])
            right = min(self.image_width-1, tlwh[0]+tlwh[2])
            bottom = min(self.image_height-1, tlwh[1]+tlwh[3])
            results.append([trk.track_id, trk.label, trk.score,
