...
from .fhog_util import pyfunc12
...
def getFeatureMaps(image, k, mapp):
	...
	#compare different impl: same mapp with no difference
	if 0: ### original implementation
		r, alfa = func1(dx.astype(np.float32), dy.astype(np.float32), boundary_x.astype(np.float32), boundary_y.astype(np.float32), height, width, numChannels) #func1 without @jit  ### 
	...
	if 1: 
		mapp['map']  = pyfunc12(dx.astype(np.float32), dy.astype(np.float32), 
						 boundary_x.astype(np.float32), boundary_y.astype(np.float32), 
						 height, width, numChannels, k)
	return mapp
...
def normalizeAndTruncate(mapp, alfa):
	...
	#diff: some sparse elems are diffent?
	if 0: 
...
def PCAFeatureMaps(mapp):
	...
	#the difference of newData is tiny(1e-8)
	if 0: ### original implementation
