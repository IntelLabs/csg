        ...
        if actor_route:
            ...
            if waypoint_list:
                ...
                for waypoint in waypoint_list:
                    print("-------- waypoint is", waypoint.attrib['x'], waypoint.attrib['y'], waypoint.attrib['z'], waypoint.attrib['speed'])
                    actor_list.append([float(waypoint.attrib['x']), float(waypoint.attrib['y']),float(waypoint.attrib['z']),float(waypoint.attrib['speed'])]) 
            #print(actor_list)
        else:
...
#########################################
    ...
    for i in range(len(actor_name_list)):
        ...
        if len(actor_pos_list[i])>2:
            ...
            positionlist = []
            actor_route=[]
            for j in range(int(len(actor_pos_list[i])/2)):
                print("j is", j)

                npc_routing_event = pyoscx.Event(actor_name+'_RoutingEvent_'+str(j),pyoscx.Priority.overwrite)
                man.add_event(npc_routing_event)

                actor_route = pyoscx.Route(actor_name+'_route_'+str(j))
                actor_routing_action = pyoscx.AssingRouteAction(actor_route)
                npc_routing_event.add_action(actor_name+'_RoutingAction_'+str(j),actor_routing_action)

                x=round(actor_pos_list[i][2*j][0], 2)
                y=round(actor_pos_list[i][2*j][1], 2)
                z=round(actor_pos_list[i][2*j][2], 2)
                positionlist.append(pyoscx.WorldPosition(x,y,z))
                actor_route.add_waypoint(pyoscx.WorldPosition(x, y, z), pyoscx.RouteStrategy.fastest)
                x=round(actor_pos_list[i][2*j+1][0], 2)
                y=round(actor_pos_list[i][2*j+1][1], 2)
                z=round(actor_pos_list[i][2*j+1][2], 2)
                positionlist.append(pyoscx.WorldPosition(x,y,z))
                actor_route.add_waypoint(pyoscx.WorldPosition(x, y, z), pyoscx.RouteStrategy.fastest)

                speed=round(actor_pos_list[i][2*j+1][3], 2)
                step_time = pyoscx.TransitionDynamics(pyoscx.DynamicsShapes.step,pyoscx.DynamicsDimension.time,1)
                npc_velocity_action = pyoscx.AbsoluteSpeedAction(speed,step_time)
                npc_routing_event.add_action(actor_name+'_VelocityAction_'+str(j), npc_velocity_action)
    
                if j==0:
                    trigcond2 = pyoscx.TraveledDistanceCondition(1)
                    trigger2 = pyoscx.EntityTrigger('start_trigger', 0, pyoscx.ConditionEdge.none, trigcond2, 'hero')
                else:        
                    trigcond2 = pyoscx.StoryboardElementStateCondition(pyoscx.StoryboardElementType.action, actor_name+'_RoutingAction_'+str(j-1), pyoscx.StoryboardElementState.completeState)
                    trigger2 = pyoscx.ValueTrigger('start_trigger', 0, pyoscx.ConditionEdge.rising, trigcond2)
                npc_routing_event.add_trigger(trigger2)
